      ...
      sections: [
        {
          ...
          lines: [
            ...
            product.sizes?.length && `Available Sizes: ${product.sizes}`,
          ].filter(Boolean),
        },
        ...(product.craftsmanship?.length
          ? [{ heading: "Craftsmanship", lines: product.craftsmanship }]
          : []),
      ],
    },
    "Care Guide": {
      ...
      sections: product.careGuide?.length
        ? [{ heading: "Care Instructions", lines: product.careGuide }]
        : [{ heading: null, lines: ["No care instructions available."] }],
    },
    Reviews: {
      ...
      <div className="border border-gray-200 rounded-2xl bg-white px-6 py-5">
        ...
        {content.sections.map((section, i) => (
          <div key={i} className={i > 0 ? "mt-6" : ""}>
            ...
            {/* Line-by-line list (Details, Care Guide) */}
            {section.lines?.length > 0 && (
              <ul className="space-y-1.5">
                {section.lines.map((line, j) => (
                  ...
                  >
                    <span className="text-[#9B72CF] mt-0.5 shrink-0">·</span>
                    {line}
                  </li>
                ))}
              </ul>
            ...
            {/* Plain text fallback (Description significance section) */}
            {section.body && !section.lines && (
